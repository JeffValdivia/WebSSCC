 ...
 */
const SERIES_STYLES = {
  "Grupo A": { header: "bg-green-500" },
  "Grupo B": { header: "bg-blue-500" },
  "Default": { header: "bg-gray-500" }
};
...
const FIXTURE_DATA_STATIC = {
  fulbito: { varones: [{ id: 1, hora: "00:00", equipoA: "Equipo A", equipoB: "Equipo B", golesA: 0, golesB: 0, rama: "VARONES" }], damas: [] },
  basket: { varones: [], damas: [] },
  voley: { damas: [], mixto: [] }
};
const STANDINGS_DATA_STATIC = {
  fulbito: { varones: [{ grupo: "Grupo de Ejemplo", teams: [{ id: 1, promo: "Equipo Ejemplo", pj: 0, pg: 0, pe: 0, pp: 0, wo: 0, gf: 0, gc: 0, dif: "0", ta: 0, tr: 0, pts: 0}] }], damas: [] },
  basket: { varones: [], damas: [] },
  voley: { damas: [], mixto: [] }
};
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.error("No se pudieron cargar los datos del servidor:", error);
        // Si falla el fetch, usamos datos estáticos como respaldo para visualización
        setFixtureData(FIXTURE_DATA_STATIC);
        setStandingsData(STANDINGS_DATA_STATIC);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const TeamCard = ({ name, align = "center", small = false }) => {
    ...
    return (
      <div className={`flex flex-col ${align === 'right' ? 'items-end' : align === 'left' ? 'items-start' : 'items-center'} gap-1`}>
        <div 
          className={`rounded-xl font-black shadow-md uppercase tracking-tight text-center ${small ? 'px-4 py-1.5 text-[14px]' : 'px-6 py-2.5 text-[20px]'}`}
  ...
  const renderStandings = (sport, gender) => {
    if (!standingsData) {
      return <p className="text-center text-slate-500">No hay datos de posiciones cargados.</p>;
    }

    const sportData = standingsData[sport];
    if (!sportData) {
      return <p className="text-center text-slate-500">No hay datos para este deporte.</p>;
    }

    const groups = sportData[gender.toLowerCase()] || [];
    
    if (!Array.isArray(groups) || groups.length === 0) {
      return <p className="text-center text-slate-500">No hay posiciones disponibles para esta categoría.</p>;
    }
    ...
    return (
      <div className="space-y-8 mt-4 pb-12">
        {groups.map((group, idx) => {
          ...
          const sStyle = SERIES_STYLES[group.grupo] || SERIES_STYLES["Default"];
          return (
            <div key={idx} className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
              <div className={`${sStyle.header} px-8 py-4 flex justify-between items-center`}>
                <div className="flex items-center gap-2">
                  <Trophy size={16} className="text-white" />
                  <h3 className="text-white font-black uppercase italic tracking-wider">{group.grupo}</h3>
                </div>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100 text-[9px] font-black uppercase tracking-widest text-slate-400">
                      ...
                      <th className="px-2 py-4 text-center">W.O.</th>
                      <th className="px-2 py-4 text-center">GF</th>
                      <th className="px-2 py-4 text-center">GC</th>
                      <th className="px-2 py-4 text-center">DIF</th>
                      <th className="px-2 py-4 text-center">TA</th>
                      <th className="px-2 py-4 text-center">TR</th>
                      ...
                      <tr key={i} className="border-b border-slate-50 last:border-none hover:bg-slate-50/50 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-4">
                            ...
                            <TeamCard name={team.promo} align="left" small />
                          </div>
                        </td>
                        <td className="px-2 py-4 text-center text-xs font-bold text-slate-500">{team.pj}</td>
                        <td className="px-2 py-4 text-center text-xs font-bold text-slate-500">{team.pg}</td>
                        <td className="px-2 py-4 text-center text-xs font-bold text-slate-500">{team.pe}</td>
                        <td className="px-2 py-4 text-center text-xs font-bold text-slate-500">{team.pp}</td>
                        <td className="px-2 py-4 text-center text-xs font-bold text-slate-500">{team.wo}</td>
                        <td className="px-2 py-4 text-center text-xs font-bold text-slate-500">{team.gf}</td>
                        <td className="px-2 py-4 text-center text-xs font-bold text-slate-500">{team.gc}</td>
                        <td className="px-2 py-4 text-center text-xs font-bold text-slate-500">{team.dif}</td>
                        <td className="px-2 py-4 text-center text-xs font-bold text-slate-500">{team.ta}</td>
                        <td className="px-2 py-4 text-center text-xs font-bold text-slate-500">{team.tr}</td>
                        <td className="px-4 py-4 text-center font-black text-[#FF0000] text-lg">{team.pts}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
  ...
  if (currentView.page === "sportDetail") {
    ...
    const matches = fixtureData[currentView.sport]?.[currentView.gender.toLowerCase()] || [];
      ...
      <div className="min-h-screen bg-[#F1F5F9] text-[#0f3654]">
        ...
        <main className="max-w-5xl mx-auto p-6 mt-6">
          ...
          <div className="grid gap-4">
            {activeTab === "fixture" ? (
               matches.length === 0 ? (
                <p className="text-center text-slate-500">No hay fixtures disponibles para esta categoría.</p>
               ) : (
                matches.map((m, i) => {
                  const sStyle = { border: "border-l-slate-200", text: "text-slate-400", bg: "bg-slate-50" };
                  return (
                    <div key={i} className={`bg-white rounded-[2.8rem] p-12 shadow-sm border border-slate-100 border-l-[12px] ${sStyle.border} flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden group hover:shadow-xl transition-all`}>
                      ...
                      <div className="flex-1"><TeamCard name={m.equipoA} align="right" /></div>
                      <div className="flex flex-col items-center px-12 border-x-2 border-slate-50">
                        <div className="flex items-center gap-6">
                          <span className="text-5xl font-black">{m.golesA}</span>
                          <div className="text-[#FF0000] font-black text-2xl italic px-2">VS</div>
                          <span className="text-5xl font-black">{m.golesB}</span>
                        </div>
                        <div className="mt-6 flex items-center gap-2 text-[12px] font-black text-black uppercase tracking-widest bg-slate-50 px-4 py-1.5 rounded-full"><Clock size={14} className="text-[#FF0000]"/> {m.hora} HRS</div>
                      </div>
                      <div className="flex-1"><TeamCard name={m.equipoB} align="left" /></div>
                    </div>
                  );
                })
               )
